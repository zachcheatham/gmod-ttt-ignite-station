...
function ENT:Use(ply)
	if IsValid(ply) and ply:IsPlayer() and ply:IsActive() then
		if ply:IsTraitor() then
			...
			ply:Ignite(5)
			ply.ignite_info = {att=self:GetPlacer(), infl=self}
			timer.Remove("ignite_station_" .. ply:UserID())
         	timer.Create("ignite_station_" .. ply:UserID(), 5.1, 1, function()
				if IsValid(ply) then
					ply.ignite_info = nil
				end
			end)
...
function ENT:OnTakeDamage(dmginfo)
	...
	if IsPlayer(att) then
		DamageLog(Format("%s damaged ignite station for %d dmg", att:Nick(), dmginfo:GetDamage()))
	end
	...
	if self:Health() < 0 then
		...
		if IsValid(self:GetPlacer()) then
			CustomMsg(self:GetPlacer(), "Your Ignite Station has been destroyed!")
		end
	end
end
